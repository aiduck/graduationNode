var projectReportSQL = {
    ...
    // admin 可以查看所有的信息
    queryAll: 'SELECT * from daily_report limit ?, ?',
    queryAllNum: 'SELECT count(*) as number FROM team',
    ...
    queryAllByStu: 'SELECT * FROM daily_report where daily_report.user_id = ? limit ?, ?',
    queryNumByStu: 'SELECT count(*) as number FROM daily_report where daily_report.user_id = ? ',
    // teacher 也只能查看参与的信息
    // report_id,report_date,report_time,report_work,report_problem,report_plan,report_status,report_comment,daily_report.project_id,daily_report.user_id 
    queryAllByTea: 'SELECT report_id,report_date,report_time,report_work,report_problem,report_plan,report_status,report_comment,daily_report.project_id,daily_report.user_id  FROM daily_report, team, classes where classes.user_id = ? and classes.class_id = team.class_id  and team.project_id = daily_report.project_id limit ?, ?',
